...
// Simulated Soil Moisture Variable
int simulated_increase = 0;
...
void loop() {
  ...
  // Update simulated increase based on pump status
  if (soil_moisture < 400) {
    digitalWrite(Pump_Pin, HIGH);
    Serial.println("Pump: ON");
    lcd.setCursor(0, 1);
    lcd.print("Pump: ON");
    simulated_increase += 10;  // Increase the simulated value
  } else {
    digitalWrite(Pump_Pin, LOW);
    Serial.println("Pump: OFF");
    lcd.setCursor(0, 1);
    lcd.print("Pump: OFF");
    simulated_increase = 0;  // Reset the simulated value when the pump is off
  }

  // Limit the simulated increase to a maximum value
  if (simulated_increase > 100) {  // Set a maximum increase limit
    simulated_increase = 100;
  }

  // Apply the simulated increase to the actual reading for display
  int displayed_soil_moisture = soil_moisture + simulated_increase;
  ...
  Serial.print(displayed_soil_moisture);
  Serial.print(" \t");
  ...
  lcd.setCursor(0, 0);
  lcd.print("M:");
  lcd.print(displayed_soil_moisture);
  lcd.setCursor(7, 0);
  lcd.print("L_T:");
